...
def _get_file_handler(level: int | str = INFO) -> FileHandler:
    logs_filename = f'logs/bankrot_scraper_{datetime.now().strftime(format="%Y-%m-%d_%H-%M-%S")}.log'
...
class BankrotLogger(Logger):
    def __init__(self, name: str = 'BankrotScraper', level: int | str = INFO):
